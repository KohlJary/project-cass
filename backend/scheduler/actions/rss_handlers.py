...
async def process_items_action(context: Dict[str, Any]) -> ActionResult:
    ...
    """
    from rss import list_unprocessed_items, mark_item_processed, get_feed
    ...
    try:
        from scheduler.actions.world_handlers import _consume_article
    except ImportError:
        ...
        return ActionResult(
            success=False,
            message="News consumption handler not available",
    ...
    for item in items:
        ...
        try:
            # Use the news consumption workflow
            result = await _consume_article(
                url=item.link,
                title=item.title,
            ...
            if result.get("success"):
                ...
                mark_item_processed(item.id, skip_reason=result.get("error", "processing_failed"))
                skipped.append({
                    "id": item.id,
                    "title": item.title,
    ...
    logger.info(f"Processed {len(processed)} RSS items, skipped {len(skipped)}")

    return ActionResult(
        success=True,
        message=f"Processed {len(processed)} item(s), skipped {len(skipped)}",
        data={
            "processed": len(processed),
            "skip_count": len(skipped),  # Using skip_count to avoid parser keyword conflict
