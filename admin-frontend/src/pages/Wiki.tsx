...
// WikiReader component - main page reader/browser view
interface WikiReaderProps {
  ...
  backlinks: { name: string; title: string; page_type: string }[];
  maturity: {
    level: number;
    depth_score: number;
...
function WikiReader({
  ...
  // Render content - simple markdown without wikilink processing in ReactMarkdown
  // Wikilinks are rendered separately in footer sections
  const renderContent = useCallback((content: string) => {
    // Remove wikilink brackets for cleaner display - the actual navigation
    // happens through the footer link sections
          ...
          <article className="reader-article">
            <header className="article-header">
              <div className="article-meta">
                ...
                {maturity && maturity.level !== undefined && (
                  <span className={`maturity-badge maturity-${maturity.level}`}>
                    {getMaturityLabel(maturity.level)}{maturity.depth_score !== undefined && ` (${maturity.depth_score.toFixed(2)})`}
                  </span>
                )}
